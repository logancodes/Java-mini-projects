...
public class TripleSum{
	...
	*/
	public static boolean TripleSum225(int[] A){
		/* ... Your code here ... */
		int [] array=new int[226]; //creates an array with size 226
		for(int i=0;i<array.length;i++){ //sets all values of the array to 226
			array[i]=226;
		}
		/* checks if the index of the new array is equal or not. if equal copy the value of the old array to new array and keep looking for the 
		...
		*/
		...
		/* gets the size of the values stored in the array upto 226. If 226 is encountered the loop stops*/
		int tracker=0;
		for(int i=0;i<array.length;i++){
			if(array[i]!=226)tracker ++;
		}
		...
		for (int i = 0; i < n; i++){
			for (int j = i; j < n; j++){
				for(int l=j;l<n;l++){
					if (array[i] + array[j]+array[l] == 225){
						return true;
					}
				}
			}
