...
public class HashTable6{
	...
	*/
	public int hash(String s){
		...
		double h=0;
		for(int i=0;i<n;i++){
			h+=Math.pow(2,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
	}
	public int hash_II(String s){
		int n=s.length();
		double h=0;
		for(int i=0;i<n;i++){
			h+=Math.pow(3,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
		
	}
	public int hash_III(String s){
		int n=s.length();
		double h=1;
		for(int i=0;i<n;i++){
			h+=Math.pow(5,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
		/*int n=s.length();
	...
	public int hash_IV(String s){
		int n=s.length();
		double h=1;
		for(int i=0;i<n;i++){
			h+=Math.pow(7,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
		/*int n=s.length();
	...
	public int hash_V(String s){
		int n=s.length();
		double h=1;
		for(int i=0;i<n;i++){
			h+=Math.pow(11,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
	}
	
	public int hash_VI(String s){
		int n=s.length();
		double h=1;
		for(int i=0;i<n;i++){
			h+=Math.pow(13,i)*s.charAt(i);
			h=h%TableSize;
		}
		return (int)h;
	}
	...
	*/
	public int insert(String s){
		...
		*/
		...
		while(T.getElement((int)res) != null){
			//res=(Math.pow(++j,2)+h);
			//res=h+h2+(j*h3);
			res=h+(j*h3)+h2;
	...
	*/
	public int find(String s){
		...
		while (true){
			String element = T.getElement((int)res);
